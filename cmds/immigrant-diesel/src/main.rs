...
fn type_ident(c: SchemaType<'_>) -> Ident {
...
fn is_copy(t: SchemaType<'_>, track: &mut Tracker) -> bool {
	let it = track.ty(t);
	match t {
		SchemaType::Scalar(s) => s.annotations.get_flag("rust", "copy", it),
		// Enums are always copy
		SchemaType::Enum(_) => true,
		SchemaType::Composite(c) => {
			for ele in c.fields() {
				if !is_copy(c.schema.schema_ty(ele.ty), track) {
					return false;
				}
			}
			true
...
fn is_immutable(column: TableColumn<'_>, track: &mut Tracker) -> bool {
	let track = track.column(column);
	// It is dangerous to update PKs, let's disable this pattern in diesel generator, at least for now.
	if column.is_pk_part() {
		return true;
	}
	column.annotations.get_flag("diesel", "immutable", track)
}

/// Fields with default values may be fully autogenerated, (e.g created_at, updated_at), or just have some default
/// for database management purposes. Lets assume it is autogenerated, unless it has diesel(manual) annotation.
fn is_autogenerated(column: TableColumn, track: &mut Tracker) -> bool {
	let track = track.column(column);
	let has_default = column.has_default();
	if column.annotations.get_flag("diesel", "manual", track) {
		assert!(
			has_default,
...
fn should_use_reference(
	...
	k != TableKind::Load
		...
		&& !(k == TableKind::Update && is_immutable(column, track))
		// default fields are skipped for New table
		&& !(k == TableKind::New && is_autogenerated(column, track))
}
fn table_has_lifetime(k: TableKind, table: &SchemaTable, track: &mut Tracker) -> bool {
	match k {
		TableKind::New | TableKind::Update => {
			let mut has_reference = false;
			for column in table.columns() {
				let copy = is_copy(column.ty(), track);
				if k == TableKind::New && is_autogenerated(column, track) {
					continue;
				}
				if should_use_reference(k, copy, column, track) {
					has_reference = true;
					break;
				}
			}
...
fn column_raw_ty(ty: SchemaType<'_>, nullable: bool, track: &mut Tracker) -> TokenStream {
	let track = track.ty(ty);
	let ty = match ty {
		SchemaType::Enum(en) => {
		...
		SchemaType::Scalar(s) => {
			let v: String = s.annotations.get_single_or_error("rust", "type", track);
...
fn column_ty(kind: TableKind, column: TableColumn<'_>, track: &mut Tracker) -> TokenStream {
	let should_use_reference =
		should_use_reference(kind, is_copy(column.ty(), track), column, track);
	let ty = column_ty_name(should_use_reference, column.ty(), track);
...
fn field_ty(field: CompositeField<'_>, track: &mut Tracker) -> TokenStream {
	let ty = column_ty_name(false, field.ty(), track);
	if field.nullable {
		quote!(Option<#ty>)
	} else {
		ty
...
fn column_ty_name(jojo_reference: bool, ty: SchemaType<'_>, track: &mut Tracker) -> TokenStream {
	let track = track.ty(ty);
	...
	match ty {
		SchemaType::Scalar(s) => {
			let native = s
				.annotations
				.get_single_or_default::<Option<String>>("rust", "native", track);
			let native_ref =
				s.annotations
					.get_single_or_default::<Option<String>>("rust", "native_ref", track);
			let custom = s.annotations.get_flag("rust", "custom", track);
...
fn print_schema() -> anyhow::Result<()> {
	...
	let rn = &rn;
	let mut tracker = Tracker::new(["rust", "diesel"]);
	let generated = generate_schema(schema, &mut report, rn, &mut tracker)?;
...
fn generate_schema(
	...
	for en in schema.enums() {
		...
		let id = enum_ident(&en);
		let track = track.enum_(en);

		let derives = en
			.annotations
			.get_multi::<String>("rust", "derive", track)
			.expect("diesel derive")
			.into_iter()
			.map(|(v, _)| syn::parse_str::<Path>(&v).unwrap());

		let serde = en.annotations.get_flag("rust", "serde", track);
	...
	for composite in schema.composites() {
		...
		let id = composite_ident(&composite);
		let track_composite = track.composite_(composite);

		let derives = composite
			.annotations
			.get_multi::<String>("rust", "derive", track_composite)
			.expect("diesel derive")
			.into_iter()
			.map(|(v, _)| syn::parse_str::<Path>(&v).unwrap());
		...
		let fields = composite
			.fields()
			.map(|f| {
				if !is_copy(f.ty(), track) {
					copy = false;
				}
				let id = field_ident(&f);
				let ty = field_ty(f, track);
				let docs = &f.docs;
				quote! {
					#(#[doc = #docs])*
					pub #id: #ty,
		...
		let raw_tys = composite
			.fields()
			.map(|f| column_raw_ty(f.ty(), f.nullable, track))
			.collect::<Vec<_>>();
		let tys = composite.fields().map(|f| field_ty(f, track));
	...
	for table in schema.tables() {
		...
		for column in table.columns() {
			...
			let ty = column_raw_ty(column.ty(), column.nullable, track);
		...
		for kind in [TableKind::New, TableKind::Load, TableKind::Update] {
			...
			let derive_attr = match kind {
				...
				TableKind::Load => {
					let derives = table
						.annotations
						.get_multi::<String>("rust", "derive", track_table)
						.expect("diesel derive")
						.into_iter()
						.map(|(v, _)| syn::parse_str::<Path>(&v).unwrap());
			...
			let extra_attrs = if kind == TableKind::Load {
				table
					.annotations
					.get_multi::<String>("rust", "attr", track_table)
			...
			let struct_lifetime = table_has_lifetime(kind, &table, track).then(|| quote!(<'a>));
			let mut columns = Vec::new();
			for column in table.columns() {
				if kind == TableKind::New && is_autogenerated(column, track) {
					continue;
				}
				let ty = column.ty();
				let track_ty = track.ty(ty);
				let diesel_attr = match ty {
					SchemaType::Scalar(s) => {
						// TODO: disallow setting both at the same time
						if let Some(s) = s.annotations.get_single_or_default::<Option<String>>(
							"diesel",
							"serialize_as",
							track_ty,
						) {
				...
				let vis = if kind == TableKind::Update && column.is_pk_part() {
					...
					quote!()
				} else if kind == TableKind::Update && is_immutable(column, track) {
				...
				let ty = column_ty(kind, column, track);
				columns.push(quote! {
					#diesel_attr
					#vis #field_name: #ty
				});
			...
			} else if kind == TableKind::Load {
				{
					...
					let pk = table
						...
						.map(|column| {
							...
							let is_reference = !is_copy(column.ty(), track);
							let reference = if is_reference {
								quote!(&)
							} else {
								Default::default()
							};
							let t = column_ty_name(is_reference, column.ty(), track);
...
mod tests {
	...
	fn test_generator(schema: &str) {
		...
		.expect("parse result");
		let mut track = Tracker::new(["diesel", "rust"]);
		let generated = generate_schema(schema, &mut report, &rn, &mut track).expect("generate");
		track.report_unused(&mut report);
